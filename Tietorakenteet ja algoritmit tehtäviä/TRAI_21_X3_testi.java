...
public class TRAI_21_X3_testi {
  ...
  static TRAI_21_X3 testattava = new TRAI_21_X3_joniveps(); /* <-- Oma tunnus tÃ¤hÃ¤n */
  ...
  public static void main(String[] args) {
    ...
    System.out.println("\nTestataan null:ja sisältäviä listoja\n");
    ...
    System.out.println("\n" + (virheet+oikein) + " satunnaisesta lisätestistä " + oikein + " oikein.");

    if (ok)
      System.out.println(
          "\nKaikki tehdyt testit antoivat oikean tuloksen.\nMuista myös itsearviointi ja aikavaativuus.");
  ...
  static boolean testaaTulosX3(int n, int dupl, boolean nulleja, int print) {
    ...
    if (print > 0)
      System.out.println(
          "Palautusarvo (poistettujen määrä) = "
              + tul
              + ". Piti poistaa "
              + dupl
    ...
    if (tul != pituus0 - L.size()) {
      ok = false;
      if (print > 0) System.out.println("Palautusarvo ei täsmää listan lyhentymiseen!");
    }
    if (tul != pituus0 - cL.size()) {
      ok = false;
      if (print > 0) System.out.println("Palautusarvo ei täsmää listan odotettuun lyhentymiseen!");
    }

    // tulostetaan tulos
    if ((L.size() <= 20 && print > 1) || print > 5) System.out.println("Jäljelle jäi   : " + L);
  ...
  static void suoritaX3satunnaiselle(int n, int aakkosto, boolean nulleja, int print) {
    ...
    // tulostetaan tulos
    if ((L.size() <= 20 && print > 1) || print > 5) System.out.println("Jäljelle jäi   : " + L);
    System.out.println("HUOM: tämä ajo ei testaa tuloksen oikeellisuutta, vaan ainoastaan tulostaa.");
