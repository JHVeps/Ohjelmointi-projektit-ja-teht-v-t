...
/**
 * Testausluokka TRA I 2021 tehtävään X1.
 */
public class TRAI_21_X1_testi {

    static Random rnd = new Random();

    static TRAI_21_X1 testattava = new TRAI_21_X1_joniveps(); /* <-- Oma tunnus tähän */
    ...
    public static void main(String[] args) {
        ...
        // testataan erilaisilla syötteillä
        ...
        ok &= testaaTulosX1(2, 300, 310, print);    // toimiihan myös isommilla luvuilla?
        ...
        // testataan monta satunnaista syötettä
        int nTest = 1000;
        int k = 0;
        int virheet = 0;
        for (k = 0; k < nTest; k++) {
        ...
        if (virheet > 0)
            ok = false;
        System.out.println("\n" + k + " lisätestistä " + (k-virheet) + " oikein.");

        if (ok)
            System.out.println("\nKaikki tehdyt testit antoivat oikean tuloksen.\n\nMuista myös itsearviointi.");
        else
    ...
    /**
     * Testaa tehtävää satunnaisella syötteellä.
     * @param n syötteen koko
     * @param min pienin kokonaisluku
     * @param max suurin kokonaisluku
     * @param print miten paljon tulostetaan 0: ei mitään, 1 jotain, 2-: enemmÃ¤n
     * @return true jos testauksen tulos oli odotettu, muuten false
     */
    static boolean testaaTulosX1(int n, int min, int max, int print) {

        // syötteen generointi, syötteestä kopio talteen ja mahdollinen tulostus
        ...
        if (print > 0 && ! samat)
            System.out.println("Väärä tulos: " + tulos + " vaikka piti olla: " + vrt);

        boolean sisaltoSailynyt = Arrays.equals(A, cA);
        if (! sisaltoSailynyt && print > 0)
            System.out.println("Virhe: metodi muuttaa taulukon sisältää!");
    ...
    static Integer[] satunnainenTaulukko(int n, int min, int max) {
        if (n < 0 || max < min || (n == 1 && min != max))
            throw new RuntimeException("Ei voi tehdä vaadittua taulukkoa");
